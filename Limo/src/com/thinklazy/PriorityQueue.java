...
/**
 * The array (or vector) implementation of a binary tree in which the 
 * children of a node at position "i" are located at positions "2i+1" and "2i+2" 
 * and whose root is at position 0, will be used to implement the complete-heap. This implementation is used because...

	The nodes are stored in level order.
	The next available position for a new node is easy to determine.
	The last leaf is easily found and removed.
 * 
...
public class PriorityQueue {
	...
	private void trickleDown(int itemPosition) {
		...
		if (2 * itemPosition + 1 >= numberOfItems) {
			...
			child = 2 * itemPosition + 1; 	// One child (at the left).     //since we have done data[0] = data[noOfItems]
