...
public class MaxBooleanSubMatric {
     ...
     * 
     * Formula is : a) Copy first row and first columns as it is from M[][] to S[][] b) For other
     * entries, use following expressions to construct S[][] If M[i][j] is 1 then S[i][j] =
     * min(S[i][j-1], S[i-1][j], S[i-1][j-1]) + 1 Else S[i][j] = 0
     * 
     */
